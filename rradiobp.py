...
# $Id: rradiobp.py,v 1.38 2017/08/09 09:27:29 bob Exp $
#
# Author : Bob Rathbone
...
def interrupt():
	...
	if radio.getTimer() and not interrupt and display_mode is radio.MODE_TIME:
		displayTime(lcd,radio)
...
def displayAirplay(lcd,radio):
	...
	if scroll == 0:
		lcd.scroll2(song,interrupt)
		if len(song) <= width:
			time.sleep(delay)
	elif scroll == 1:
		lcd.scroll2(album,interrupt)
		if len(album) <= width:
			time.sleep(delay)
	return
