...
class LCDScanf(object):
    ...
    *in*). Move the navigation switch side to side (switches 6 and 7) to
    ...
    For example:
        ...
        >>> scanner = tspifacecad.tools.LCDScanf("Text: %c%2i%.%r")
        >>> print(scanner.scan())  # user enters things on Tspifacecad
        ['a', '13', '!']

    You can also specify custom values:

        >>> scanner = tspifacecad.tools.LCDScanf(
        ...     "Animal: %m%r",
    ...
    def scan(self):
        ...
        # wait for user input
        listener = tspifacecad.SwitchEventListener(self.cad)
        listener.register(7, tspifacecad.IODIR_ON, self.right_event)
        listener.register(6, tspifacecad.IODIR_ON, self.left_event)
        listener.register(5, tspifacecad.IODIR_ON, self.change_mode_event)
        listener.register(4, tspifacecad.IODIR_ON, self.return_string_event)
